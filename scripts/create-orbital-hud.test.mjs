...
import { enhanceProfileSvg, fetchLiveStats } from './create-orbital-hud.mjs';
...
test('fetches requested user and aggregates ranges longer than one year', async (t) => {
  ...
  const stats = await fetchLiveStats('AltesHaus', 'test-token', '2025-09-07', '2026-09-11');
...
test('GraphQL errors fail instead of publishing zero counts', async (t) => {
  ...
  await assert.rejects(fetchLiveStats('AltesHaus', 'test-token', '2026-01-01', '2026-09-11'), /Denied/);
});
